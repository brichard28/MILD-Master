...
% %     p1 = shadedErrorBar(single_onset_time,nanmean(this_target_data,1),std(this_target_data,[],1)./(sqrt(num_subjects - 1)),'lineProps',{'-'});
% %     p2 = shadedErrorBar(single_onset_time,nanmean(this_masker_data,1),std(this_masker_data,[],1)./(sqrt(num_subjects - 1)),'lineProps',{'--'});
% %     ylim([ymin,ymax])
...
% %     xline(0,':k','LineWidth',2)
% %     xline(250,':k','LineWidth',2)
% %     legend([p1(1),p2(1)],{'Target','Masker'})
% % 
% % 
...
for i = 1:length(curr_channel_indices)
    ...
    this_target_data = squeeze(nanmean(itd5_lead_bash_target_lag_nonbash_masker(:,curr_channel_index,:),2));
    this_masker_data = squeeze(nanmean(itd5_lead_bash_masker_lag_nonbash_target(:,curr_channel_index,:),2));
    p1 = plot(single_onset_time,nanmean(this_target_data,1),'Color',colors(1,:),'LineStyle','-','LineWidth',1.5);
    p2 = plot(single_onset_time,nanmean(this_masker_data,1),'Color',colors(1,:),'LineStyle','--','LineWidth',1.5);
    ylim([ymin,ymax])
    xlim([erp_window_start_time,erp_window_end_time])
    ylabel('Voltage (uV)','FontSize',18)
    title('Bash Leads','FontSize',18)
    ...
    this_target_data = squeeze(nanmean(itd5_lead_nonbash_masker_lag_bash_target(:,curr_channel_index,:),2));
    this_masker_data = squeeze(nanmean(itd5_lead_nonbash_target_lag_bash_masker(:,curr_channel_index,:),2));
    p1 = plot(single_onset_time,nanmean(this_target_data,1),'Color',colors(2,:),'LineStyle','-','LineWidth',1.5);
    p2 = plot(single_onset_time,nanmean(this_masker_data,1),'Color',colors(2,:),'LineStyle','--','LineWidth',1.5);
    ylim([ymin,ymax])
    xlim([erp_window_start_time,erp_window_end_time])
    title('Bash Lags','FontSize',18)
    xline(0,':k','LineWidth',2)
    ...
    this_target_data = squeeze(nanmean(itd5_lead_nonbash_target_lag_nonbash_masker(:,curr_channel_index,:),2));
    this_masker_data = squeeze(nanmean(itd5_lead_nonbash_masker_lag_nonbash_target(:,curr_channel_index,:),2));
    p1 = plot(single_onset_time,nanmean(this_target_data,1),'Color',colors(3,:),'LineStyle','-','LineWidth',1.5);
    p2 = plot(single_onset_time,nanmean(this_masker_data,1),'Color',colors(3,:),'LineStyle','--','LineWidth',1.5);
    ylim([ymin,ymax])
    xlim([erp_window_start_time,erp_window_end_time])
    title('No Bash','FontSize',18)
    xline(0,':k','LineWidth',2)
...
curr_channel_indices = {[31,32],[parietooccipital_channels]}; % Fz, Cz, and Pz
for i = 1:length(curr_channel_indices)
    curr_channel_index = cell2mat(curr_channel_indices(i));
        figure;
    ...
    p1 = plot(single_onset_time,nanmean(this_bash_leads_data,1),'Color',colors(1,:),'LineWidth',2);
    p2 = plot(single_onset_time,nanmean(this_bash_lags_data,1),'Color',colors(2,:),'LineWidth',2);
    p3 = plot(single_onset_time,nanmean(this_no_bash_data,1),'Color',colors(3,:),'LineWidth',2);
    ylim([ymin,ymax])
    xlim([erp_window_start_time,erp_window_end_time])
    ylabel('Voltage (uV)','FontSize',18)
