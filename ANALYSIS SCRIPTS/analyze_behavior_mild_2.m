...
BehaviorTable = readtable(append(mild_master_root,'RESULTS DATA\MILD-2 Behavior Files\mild-2.xlsx'),'Format','auto');

subject_ID = char('CDARHUC984HJ');% mild bi-ci
num_conditions = 8;
...
for isubject = 1:size(subject_ID,1) % For each subject...
    ...
    WordTimesTable = readtable(append(mild_master_root,"RESULTS DATA\MILD-2 Behavior Files\mild-2__s_" + strtrim(string(subject_ID(isubject,:))) + "__Word_Times.csv"));
    ...
    all_maskers = {'side=r_masker=speech_itd=0_ild=0_az=10_mag=0_control=0',...
        'side=l_masker=speech_itd=0_ild=0_az=10_mag=0_control=0',...
        'side=r_masker=speech_itd=0_ild=0_az=30_mag=0_control=0',...
        'side=l_masker=speech_itd=0_ild=0_az=30_mag=0_control=0',...
        'side=r_masker=speech_itd=0_ild=0_az=10_mag=1_control=0',...
        'side=l_masker=speech_itd=0_ild=0_az=10_mag=1_control=0',...
        'side=r_masker=speech_itd=0_ild=0_az=30_mag=1_control=0',...
        'side=l_masker=speech_itd=0_ild=0_az=30_mag=1_control=0'};
    small_itd_cond = [5,8];
    large_itd_cond = [1,4];
    small_ild_cond = [2,6];
    large_ild_cond = [3,7];
    ...
    for itrial = 1:length(rows_this_subject)
        ...
        % convert percentage
        this_trial_click_times = ((this_trial_click_times/100)*(dur));
        ...
        if contains(which_soundfile_this_trial, '_s')
        ...
        this_trial_target_all = WordTimesTable(WordTimesTable.Run == which_soundfile_this_trial & string(WordTimesTable.Condition) == this_trial_masker & string(WordTimesTable.Type) == 'Target',4:end);
        this_trial_target_words = table2array(this_trial_target_all(:,1:2:end));
        this_trial_target_times = (table2array(this_trial_target_all(:,2:2:end))); % have to add cue back in

        this_trial_masker_all = WordTimesTable(WordTimesTable.Run == which_soundfile_this_trial & string(WordTimesTable.Condition) == this_trial_masker & string(WordTimesTable.Type) == 'Masker',4:end);
        ...
        threshold_window_start = 0.5; %0.2
