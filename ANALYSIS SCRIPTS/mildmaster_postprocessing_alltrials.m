...
for isubject = 1:size(curr_subject_ID,1)
    ...
    ERP_info = struct('SubID',{},'Trial',{},'Lead_Stream',{},'Lag_Stream',{},'Lead_Word',{},'Lag_Word',{},'Target_Direction',{},'Condition',{},'Responded',{});
...
%         end
        ...
        for iclick = 1:length(this_trial_click_times) % for each target word onset...
            ...
            % Reject ERPs with amplitude above +/- 100 uV
            if any(abs(this_erp) > noise_thresh,'all')
                %disp('ERP rejected')
        ...
        for ionset = 2:length(this_trial_pair_onset_times) % for each word pair onset, excluding the first
            ...
            if ~isempty(ERP_info)
             ...
             % If there was a click in response to this word, save it
            % as such
            ...
            double_click_threshold = threshold_window_end - threshold_window_start; % distance between clicks at which it would be decided that it is a double click
            current_click_times = this_trial_click_times/1000 + 0.702;
            ...
            current_click_distances = current_click_times - this_trial_pair_onset_times(ionset); % ...find the time index of that click...
            ...
            if sum(threshold_window_start <= current_click_distances & current_click_distances <= threshold_window_end) > 0
                ERP_info.Responded = [ERP_info.Responded; 1];
            else
                ERP_info.Responded = [ERP_info.Responded; 0];
            end

            % take this click out of the running
            current_click_times(threshold_window_start <= current_click_distances & current_click_distances <= threshold_window_end) = [];
    ...
    disp(append(string(num_rejected)," ERPs rejected"))
    %% SAVE INFO FOR THIS SUBBY
    save(append('C:\Users\benri\Downloads\Results_Subject_',strtrim(string(curr_subject_ID(isubject,:))),'_yes_button_press.mat'),'data_by_pair_onset_baselined','data_by_button_press_baselined','ERP_info_button_press','ERP_info','-v7.3')
