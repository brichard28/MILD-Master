...
for isubject = 1:size(curr_subject_ID,1)
    ...
    for itrial = 1:size(this_EEG.data,3)% for each trial (should be 144)
        ...
        for ionset = 1:length(this_trial_masker_times)
            ...
            if ~isempty(ERP_info_masker)
                ...
                ERP_info_target.Word = [ERP_info_masker.Word; this_trial_masker_words(ionset)];
    ...
    all_itd50_by_target_onset(isubject,:,:) = squeeze(mean(data_by_target_onset_baselined(:,:,logical(ismember(ERP_info_target(:).Condition,[2,5]))),3));
    all_itd50_by_masker_onset(isubject,:,:) = squeeze(mean(data_by_masker_onset_baselined(:,:,logical(ismember(ERP_info_masker(:).Condition,[2,5]))),3));
    
    all_itd500_by_target_onset(isubject,:,:) = squeeze(mean(data_by_target_onset_baselined(:,:,logical(ismember(ERP_info_target(:).Condition,[1,6]))),3));
    all_itd500_by_masker_onset(isubject,:,:) = squeeze(mean(data_by_masker_onset_baselined(:,:,logical(ismember(ERP_info_masker(:).Condition,[1,6]))),3));
    
    all_ild5_by_target_onset(isubject,:,:) = squeeze(mean(data_by_target_onset_baselined(:,:,logical(ismember(ERP_info_target(:).Condition,[3,8]))),3));
    all_ild5_by_masker_onset(isubject,:,:) = squeeze(mean(data_by_masker_onset_baselined(:,:,logical(ismember(ERP_info_masker(:).Condition,[3,8]))),3));
    
    all_ild5mag_by_target_onset(isubject,:,:) = squeeze(mean(data_by_target_onset_baselined(:,:,logical(ismember(ERP_info_target(:).Condition,[4,7]))),3));
    all_ild5mag_by_masker_onset(isubject,:,:) = squeeze(mean(data_by_masker_onset_baselined(:,:,logical(ismember(ERP_info_masker(:).Condition,[4,7]))),3));
    

    %% Plot frontocentral erp for each subject
    figure;
    subplot(1,4,1)
    hold on
    p1 = plot(single_onset_time,squeeze(mean(all_itd50_by_target_onset(isubject,frontocentral_channels,:),2)),'-r');
    p2 = plot(single_onset_time,squeeze(mean(all_itd50_by_masker_onset(isubject,frontocentral_channels,:),2)),'-b');
    ...
    subplot(1,4,2)
    hold on
    p1 = plot(single_onset_time,squeeze(mean(all_itd500_by_target_onset(isubject,frontocentral_channels,:),2)),'-r');
    p2 = plot(single_onset_time,squeeze(mean(all_itd500_by_masker_onset(isubject,frontocentral_channels,:),2)),'-b');
    ...
    subplot(1,4,3)
    hold on
    p1 = plot(single_onset_time,squeeze(mean(all_ild5_by_target_onset(isubject,frontocentral_channels,:),2)),'-r');
    p2 = plot(single_onset_time,squeeze(mean(all_ild5_by_masker_onset(isubject,frontocentral_channels,:),2)),'-b');
    ...
    subplot(1,4,4)
    hold on
    p1 = plot(single_onset_time,squeeze(mean(all_ild5mag_by_target_onset(isubject,frontocentral_channels,:),2)),'-r');
    p2 = plot(single_onset_time,squeeze(mean(all_ild5mag_by_masker_onset(isubject,frontocentral_channels,:),2)),'-b');
   ...
   %% Plot whole trial EEG
    ...
    for itopo = topoplot_indices
        subplot(8,length(topoplot_indices)+ 1,iplot);
        this_data = mean(all_itd50_by_target_onset(:,:,itopo), [1,3]);
    ...
    for itopo = topoplot_indices
        subplot(8,length(topoplot_indices)+ 1,iplot);
        this_data = mean(all_itd50_by_masker_onset(:,:,itopo), [1,3]);
        topoplot(this_data,this_EEG.chanlocs,'maplimits',[cmin, cmax]);
        title([num2str(topoplot_times(itime)),' ms'])
        iplot = iplot + 1;
        itime = itime + 1;
    ...
    for itopo = topoplot_indices
        subplot(8,length(topoplot_indices)+ 1,iplot);
        this_data = mean(all_itd500_by_target_onset(:,:,itopo), [1,3]);
        topoplot(this_data,this_EEG.chanlocs,'maplimits',[cmin, cmax]);
        title([num2str(topoplot_times(itime)),' ms'])
        iplot = iplot + 1;
        itime = itime + 1;
    ...
    for itopo = topoplot_indices
        subplot(8,length(topoplot_indices)+ 1,iplot);
        this_data = mean(all_itd500_by_masker_onset(:,:,itopo), [1,3]);
        topoplot(this_data,this_EEG.chanlocs,'maplimits',[cmin, cmax]);
        title([num2str(topoplot_times(itime)),' ms'])
        iplot = iplot + 1;
        itime = itime + 1;
    ...
    for itopo = topoplot_indices
        subplot(8,length(topoplot_indices)+ 1,iplot);
        this_data = mean(all_ild5_by_target_onset(:,:,itopo), [1,3]);
        topoplot(this_data,this_EEG.chanlocs,'maplimits',[cmin, cmax]);
        title([num2str(topoplot_times(itime)),' ms'])
        iplot = iplot + 1;
        itime = itime + 1;
    ...
    for itopo = topoplot_indices
        subplot(8,length(topoplot_indices)+ 1,iplot);
        this_data = mean(all_ild5_by_masker_onset(:,:,itopo), [1,3]);
        topoplot(this_data,this_EEG.chanlocs,'maplimits',[cmin, cmax]);
        title([num2str(topoplot_times(itime)),' ms'])
        iplot = iplot + 1;
        itime = itime + 1;
    ...
    for itopo = topoplot_indices
        subplot(8,length(topoplot_indices)+ 1,iplot);
        this_data = mean(all_ild5mag_by_target_onset(:,:,itopo), [1,3]);
        topoplot(this_data,this_EEG.chanlocs,'maplimits',[cmin, cmax]);
        title([num2str(topoplot_times(itime)),' ms'])
        iplot = iplot + 1;
        itime = itime + 1;
    ...
    for itopo = topoplot_indices
        subplot(8,length(topoplot_indices)+ 1,iplot);
        this_data = mean(all_ild5mag_by_masker_onset(:,:,itopo), [1,3]);
        topoplot(this_data,this_EEG.chanlocs,'maplimits',[cmin, cmax]);
        title([num2str(topoplot_times(itime)),' ms'])
        iplot = iplot + 1;
        itime = itime + 1;
