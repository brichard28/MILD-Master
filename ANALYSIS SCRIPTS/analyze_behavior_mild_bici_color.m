...
subject_ID = char('mild-bici-color-1_snr-4');% mild bi-ci
num_conditions = 36;
...
for isubject = 1:size(subject_ID,1) % For each subject...
    ...
    for itrial = 1:length(rows_this_subject)
        ...
        this_trial_target_all = WordTimesTable(string(WordTimesTable.Run) == this_trial_soundfile & string(WordTimesTable.Type) == 'Target',4:end);
        ...
        this_trial_masker_all = WordTimesTable(string(WordTimesTable.Run) == this_trial_soundfile & string(WordTimesTable.Type) == 'Masker',4:end);
