...
BehaviorTable = readtable(append(mild_master_root,'RESULTS DATA\MILD-BICI Behavior Files\mild-bici.xlsx'),'Format','auto');

subject_ID = char('mild_bici_1','mild_bici_2');% mild bi-ci
num_conditions = 8;
...
for isubject = 1:size(subject_ID,1) % For each subject...
    ...
    WordTimesTable = readtable(append(mild_master_root,"RESULTS DATA\MILD-BICI Behavior Files\mild-bici__s_" + strtrim(string(subject_ID(isubject,:))) + "__Word_Times.csv"));
    ...
    all_maskers = {'side=r_itd=0_az=60_az_itd=0_mag=1_lpf=0',...
'side=r_itd=0_az=90_az_itd=0_mag=0_lpf=0',...
'side=r_itd=0_az=90_az_itd=0_mag=1_lpf=0',...
'side=l_itd=0_az=60_az_itd=0_mag=1_lpf=0',...
'side=l_itd=0_az=60_az_itd=0_mag=0_lpf=0',...
'side=l_itd=0_az=90_az_itd=0_mag=0_lpf=0',...
'side=l_itd=0_az=90_az_itd=0_mag=1_lpf=0',...
'side=r_itd=0_az=60_az_itd=0_mag=0_lpf=0'};
    small_itd_cond = [5,8];
    large_itd_cond = [1,4];
    small_ild_cond = [2,6];
    large_ild_cond = [3,7];
    ...
    for itrial = 1:length(rows_this_subject)
        ...
        threshold_window_start = 0; %0.2
        threshold_window_end =  3; % 1.0
        tVec = 0:1/44100:16;
    ...
    figure(rt_fig)
    subplot(round(size(subject_ID,1)),8,isubject)
