...
#    workdir = '/home/cbrown/work/Python'
    ...
    exp.stim.fs = 44100.
    exp.stim.basedir = os.path.join("stim","bobmike",exp.name,f"s_{exp.subjID}")
    exp.stim.stimfiles = {}
    ...
    if triggers:
        ...
        ntriggers = 3
        exp.stim.trigfile = os.path.join(workdir,'data',f"{exp.name}",f'{exp.name}__s_{exp.subjID}__{time.strftime("%Y-%m-%d")}__triggers.csv')
        ...
        exp.stim.trigfile_cond = os.path.join(workdir,'data',f"{exp.name}",f'{exp.name}__s_{exp.subjID}__{time.strftime("%Y-%m-%d")}__condition_order.csv')
