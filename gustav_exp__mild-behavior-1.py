...
#    workdir = '/home/cbrown/work/Python'
    ...
    if triggers:
        ...
        exp.stim.trigger_dict = {'Inhale': 1, 'Exhale': 2, 'Hold_Breath': 3, 'Button_Press': 4}
        ntriggers =  4
        ...
        for f in os.scandir(exp.stim.basedir):
            if f.is_dir() and f.name != "practice":
                ...
                tf.write(f"{exp.subjID},{exp.var.factorial['masker'].index(f.name) + 5},{f.name}\n")
...
def pre_exp(exp):
    try:
        ...
        exp.interface.update_Prompt("Wait for the experimenter to begin", show=True, redraw=True)
        ret = exp.interface.get_resp()

        practice = True
        while practice:
            exp.interface.update_Prompt("More Practice? (y/N)", show=True, redraw=True)
            ret = exp.interface.get_resp()
            if ret != 'y':
...
🬇🬋🬫🬋🬋
  🬉  """, show=True, redraw=True)
                ...
                if not exp.debug:
                    ...
                    start_ms = exp.interface.timestamp_ms()
                    this_elapsed_ms_extended = 0
                    while this_elapsed_ms_extended < exp.stim.wait_for_last_response_ms:
                        this_elapsed_ms_extended = exp.interface.timestamp_ms() - start_ms
                        ret = exp.interface.get_resp(timeout=this_wait_ms/1000)
                        this_current_ms = exp.interface.timestamp_ms()
                        this_elapsed_ms = this_current_ms - start_ms
                        #this_elapsed_percent = this_elapsed_ms / dur_ms * 100
                        ...
                        if ret and ret in ['p']:
                            ...
                            #responses.append(str(this_elapsed_ms_extended + this_elapsed_ms))
                            resp_percent.append(this_elapsed_percent)
                ...
                for i in range(exp.stim.breath_blocks):
                    ...
                    if ret == exp.quitKey:
                        ...
                        while hale_cur > 0:
                            ...
                            time.sleep(.2)
        if not ret == exp.quitKey:
            exp.interface.update_Prompt("Button Press Exercise? (y/N)", show=True, redraw=True)
            ret = exp.interface.get_resp()
            if ret != 'y':
                exp.interface.update_Prompt("No Button Press Exercise", show=True, redraw=True)
            else:
                exp.interface.update_Prompt("Now we will begin the button press exercise. \n When you see the fixation cross, you may begin pressing. \n Hit a number key to begin.", show=True, redraw=True)
                wait = True
                while wait:
                    ret = exp.interface.get_resp()
                    if ret.isdigit():
                ...
                button_file = f'data/{exp.name}/{exp.name}__s_{exp.subjID}__button_press_times.csv'
                if os.path.exists(button_file):
                    df = open(button_file, 'a+')
                else:
                    df = open(button_file, 'a+')
                    df.write("Responses\n")
                df.close()
                ...
                finished_with_button = False
                button_count = 0
                responses = []
                fs = 44100
                out = np.zeros((2*fs,2))
                trigs = audio_triggers(out.shape[0], fs, {0: 1}, nbits = 1, click_len = 2, zeropad_cols = 0)
                out = np.hstack((out, trigs))
                s = exp.stim.audiodev.open_array(out,fs)

                if triggers:
                    s.play()
                    exp.user.triggers.trigger(exp.stim.trigger_dict['Button_Press'])
...
🬇🬋🬫🬋🬋
  🬉  """, show=True, redraw=True)
                ...
                this_button = False
                this_button_time = 0
                this_button_time_prev = 0
                bad_button = 0
                bad_button_time = 0
                no_clicks_time = 0
                while not finished_with_button:
                    ret = exp.interface.get_resp(timeout=this_wait_ms/1000)
                    this_current_ms = exp.interface.timestamp_ms()
                    this_elapsed_ms = this_current_ms - start_ms
                    #exp.interface.update_Prompt(str(start_time_start - end_time_trigger),redraw=True)
                    if ret:
                        this_button_time = this_elapsed_ms
                        #exp.interface.update_Prompt(str(this_button_time),redraw=True)
                        if this_button:
                            this_button = True
                            if this_button_time > this_button_time_prev + 1000:
                                this_button = False
                                button_count += 1
                                responses.append(str(this_button_time_prev))
                                this_button_time_prev = this_button_time
                            else:
                                this_button = False
                                this_button_time = 0
                                bad_button += 1
                                if bad_button > 5:
                                    exp.interface.update_Status_Center("Please slow down", redraw=True)
                                    bad_button = 0
                                    bad_button_time = this_elapsed_ms

                        else:
                            this_button = True
                            this_button_time_prev = this_button_time
                    elif this_button == True and this_elapsed_ms > this_button_time_prev + 1000:
                        this_button = False
                        button_count += 1
                        responses.append(str(this_button_time_prev))
                        this_button_time_prev = this_button_time
                    ...
                    if this_elapsed_ms > bad_button_time + 3000:
                        exp.interface.update_Status_Center("", redraw=True)
                        bad_button_time = 0
                    
                    if this_elapsed_ms > this_button_time_prev + 10000:
                        exp.interface.update_Status_Left("No clicks in previous 10 seconds", redraw = True)
                        no_clicks_time = this_elapsed_ms

                    if this_elapsed_ms > no_clicks_time + 3000:
                        exp.interface.update_Status_Left("", redraw = True)
                        no_clicks_time = 0


                    # disp += str(button_count)
                    # exp.interface.update_Status_Right(disp, redraw=True)
                        

                    if button_count == 150:
                        finished_with_button = True
                        # # wait 1 second before they can start again

                        # this_pause_ms = 1000
                        # wait_start_ms = exp.interface.timestamp_ms()
                        # wait = True
                        # while wait:
                        #     wait_ret = exp.interface.get_resp(timeout=this_pause_ms/1000)
                        #     this_current_ms = exp.interface.timestamp_ms()
                        #     wait_time_elapsed = this_current_ms - wait_start_ms

                        #     if wait_ret and len(responses) != 0:
                        #         button_count -= 1
                        #         responses.pop()
                        #         wait = False

                        #     if wait_time_elapsed == this_pause_ms:
                        #         wait = False
                ...
                exp.interface.update_Prompt("All Done!", show=True, redraw=True)
                df = open(button_file, 'a+')
                df.write(f"{responses}\n")
                df.close()
...
def pre_block(exp):
    try:
        ...
        exp.user.block_pc = 0.
        exp.user.pract = 1
        exp.interface.update_Status_Left(f"Block {exp.run.block+1} of {exp.run.nblocks}", redraw=False)
    except Exception as e:
        exp.interface.destroy()
        raise e
...
▀▀▀▀🭞🭜🭘""", show=True, redraw=True)
            ...
            vcue_start_ms = exp.interface.timestamp_ms()
            ...
            while s.is_playing:
                ...
                if exp.interface.timestamp_ms() - vcue_start_ms > 1000:
                    exp.interface.update_Prompt("""  🬦  
🬇🬋🬫🬋🬋
  🬉  """, show=True, redraw=True)
                #exp.interface.update_Status_Right(f"{resp_percent}", redraw=True)

            start_ms_extended = exp.interface.timestamp_ms()
            this_elapsed_ms_extended = 0
            while this_elapsed_ms_extended < exp.stim.wait_for_last_response_ms:
                this_elapsed_ms_extended = exp.interface.timestamp_ms() - start_ms_extended
                ret = exp.interface.get_resp(timeout=this_wait_ms/1000)
                #this_elapsed_percent = this_elapsed_ms / dur_ms * 100
                #this_elapsed_percent = (exp.interface.timestamp_ms() - start_ms) / dur_ms * 100
                if ret and ret in ['p']:
                    ...
                    responses.append(str(this_elapsed_ms_extended + start_ms_extended - start_ms))
    ...
    if exp.user.break_after_run:
                ...
                while wait:
                    ...
                    if ret:
                        ...
                        exp.run.gustav_is_go = False
                        wait = False
    exp.interface.update_Prompt("", show=False, redraw=False)
...
def post_exp(exp):
    try:
        exp.interface.update_Prompt("All Done! Click a key to exit", show = True, redraw = True)
        wait = True
        ret = ''
        while wait:
            ret = exp.interface.get_resp()
